...
module.exports = async (deployer, network, [DAO, treasury, liquidity]) => {
  await deployer.deploy(
    MCL,
    network === 'hartest' ? 'https://meta-avatar-test.marscolony.io/' : 'https://meta-avatar.marscolony.io/',
  );
  await deployProxy(AvatarManager, [
    DAO,
    MCL.address,
  ], { deployer });
  ...
  if (network === 'hartest' || network === 'development') {
    ...
    await _AvatarManager.setGameManager('0xc65F8BA708814653EDdCe0e9f75827fe309E29aD');
    _GM = await GM.at('0xc65F8BA708814653EDdCe0e9f75827fe309E29aD');
  } else { // harmain
    await _AvatarManager.setGameManager('0x0D112a449D23961d03E906572D8ce861C441D6c3');
