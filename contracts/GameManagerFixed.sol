...
contract GameManagerFixed is IGameManager, PausableUpgradeable, Constants {
    ...
    event MissionReward(
        uint256 indexed landId, uint256 indexed avatarId, uint256 indexed rewardType, uint256 rewardAmount
    );
    ...
    function proceedFinishMissionMessage(string calldata message) private {
        (ICollectionManager collectionManager, IMartianColonists martianColonists, ILootboxes lootboxes, ICLNY clny) =
            d.getCmMclLbClny();
    ...
    function finishMission(string calldata message, uint8 v, bytes32 r, bytes32 s) external {
    ...
    function mintAvatar() external nonReentrant {
        ...
        clny.mint(
            0x2581A6C674D84dAD92A81E8d3072C9561c21B935, (AVATAR_MINT_COST * 10 ** 18 * 3) / 100, REASON_CREATORS_ROYALTY
        );
    ...
    function getEarned(uint256 tokenId) public view returns (uint256) {
        return (getPassiveEarningSpeed(tokenId) * (block.timestamp - tokenData[tokenId].lastCLNYCheckout) * 10 ** 18)
            / (24 * 60 * 60) + tokenData[tokenId].fixedEarnings + landMissionEarnings[tokenId];
    }
