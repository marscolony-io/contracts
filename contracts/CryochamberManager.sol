...
contract CryochamberManager is GameConnection, PausableUpgradeable, ICryochamber {
  ...
  function purchaseCryochamberEnergy(address user, uint256 _energyAmount)
    external
    override
    onlyGameManager
    hasCryochamber(user)
  ...
  function isAvatarInCryoChamber(uint256 avatarId) public view override returns (bool) {
    CryoTime memory avatarCryo = cryos[avatarId];
    return avatarCryo.endTime > 0 && avatarCryo.endTime > uint64(block.timestamp);
  }

  function timeToStayInCryoChamber(uint256 avatarId) public view override returns (uint256) {
    CryoTime memory avatarCryo = cryos[avatarId];
  ...
  function putAvatarInCryochamber(uint256 avatarId, address user) private {
    ...
    cryos[avatarId] = CryoTime(
      uint64(block.timestamp) + cryoPeriodLength,
      (estimateXpAddition(avatarId) * cryoPeriodLength) / (24 * 60 * 60)
    );
  }
  ...
  function getAvatarCryoStatus(uint256 avatarId) public view override returns (CryoTime memory) {
