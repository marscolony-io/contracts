...
contract MissionManager is GameConnection, PausableUpgradeable {
  ...
  function getRevshare(address _address) public view returns (uint8 revShare) {
  ...
  function getRevshareForLands(uint256[] memory tokenIds) external view returns (uint8[] memory) {
  ...
  function _getLandData(uint256 landId) private view returns (LandData memory) {
    ...
    uint8 transportHubLevel = d.gameManager().getAttributesMany(lands)[0].transport;

    return LandData(availableMissionCount, landOwner, isPrivate, revshare, transportHubLevel);
  }
